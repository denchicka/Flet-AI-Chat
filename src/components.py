...
class NotificationSelector(ft.Dropdown):
    """
       Выпадающий список для выбора отправки уведомлений.

       Наследуется от ft.Dropdown для создания кастомного выпадающего списка.

       Args:
           notifications (list): Список доступных форматов отправки:
    ...
    def __init__(self, notifications: list):
        ...
        # Настройка внешнего вида выпадающего списка
        self.label = None  # Убираем текстовую метку
        self.hint_text = "Куда отправлять уведомления"  # Текст-подсказка

        # Создание списка опций из предоставленных моделей
        self.options = [
            ft.dropdown.Option(
                key=notification['key'],  # Ключ уведомлений как канал
                text=notification['name']  # Название куда отправлять уведомления как отображаемый текст
        ...
        # Установка начального значения (первый выбор из списка уведомлений)
        self.value = notifications[0]['key'] if notifications else None
